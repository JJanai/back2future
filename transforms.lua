...
function M.ColorNormalize(meanstd)
   return function(img)
      img = img:clone()
      local chs = (img:size(1) / 3) - 1
      for c=0,chs do
        for i=1,3 do
           img[3*c + i]:add(-meanstd.mean[i])
           img[3*c + i]:div(meanstd.std[i])
        end
      end 
      return img
   end
...
function M.Lighting(alphastd, eigval, eigvec)
   return function(input)
      ...
      input = input:clone()
      local chs = (input:size(1) / 3) - 1
      for c=0,chs do
        for i=1,3 do
           input[3*c + i]:add(rgb[i])
        end
      end 
      return input
   end
...
function M.Gamma(var)
   return function(input)
      local gamma = torch.normal(1.0, var)
...
function M.Saturation(var)
   ...
   return function(input)
      ...
      local chs = input:size(1) / 3 - 1
      for c=0,chs do
        grayscale(gs[{{3*c + 1,3*c + 3},{},{}}], input[{{3*c + 1,3*c + 3},{},{}}])
      end

      local alpha = 1.0 + torch.normal(0, var)
...
function M.Contrast(var)
   ...
   return function(input)
      ...
      for c=0,chs do
        gs[{{3*c + 1,3*c + 3},{},{}}]:fill(gs[3*c + 1]:mean())   
      end
