...
function myCmdLine:__readArgument__(params, arg, i, nArgument)
...
function myCmdLine:__readOption__(params, arg, i)
   local key = arg[i]
...
function myCmdLine:__init(argseparator_,keyseparator_)
...
function myCmdLine:silent()
   self.silentio = true
end

function myCmdLine:addTime(name, format)
...
function myCmdLine:string(prefix, params, ignore)
   local arguments = {}
   local options = {}
   prefix = prefix or ''
...
local oprint = nil
function myCmdLine:log(file, params)
   local f = (io.type(file) == 'file' and file) or io.open(file, 'a')
...
function myCmdLine:text(txt)
   txt = txt or ''
   assert(type(txt) == 'string')
   table.insert(self.helplines, txt)
end

function myCmdLine:help(arg)
   io.write('Usage: ')
